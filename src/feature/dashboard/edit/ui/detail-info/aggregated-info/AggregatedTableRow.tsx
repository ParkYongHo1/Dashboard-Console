...
import { CONDITION } from "../../../model/constants";
import { METHOD } from "@/feature/dashboard/add/model/constants";

interface AggregatedTableRowProps {
  item: AggregatedData;
  index: number;
  databaseColumn: DatabaseColumn[];
  onItemChange: (id: number, field: keyof AggregatedData, value: string | number) => void;
  onDeleteItem: (id: number) => void;
  tableRef: RefObject<HTMLTableElement | null>;
}

const AggregatedTableRow: React.FC<AggregatedTableRowProps> = ({ item, index, databaseColumn, onItemChange, onDeleteItem, tableRef }) => {
  ...
  return (
    <Draggable key={item.aggregatedId} draggableId={item.aggregatedId.toString()} index={index}>
      {(provided, snapshot) => (
        <tr
        ...
        >
          ...
          <td className="py-2 px-2 border border-gray-300">
            <select
              ...
              onChange={(e) => {
                onItemChange(item.aggregatedId, "aggregatedDatabaseColumn", e.target.value);
              }}
            >
          ...
          <td className="py-2 px-2 border border-gray-300">
            <input
              ...
              onChange={(e) => onItemChange(item.aggregatedId, "databaseColumnAlias", e.target.value)}
            />
          </td>
          <td className="py-2 px-2 border border-gray-300">
            <select
              ...
              onChange={(e) => onItemChange(item.aggregatedId, "dashboardCondition", e.target.value)}
            >
          ...
          <td className="py-2 px-2 border border-gray-300">
            <select
              ...
              onChange={(e) => onItemChange(item.aggregatedId, "statMethod", e.target.value)}
            >
          ...
          <td className="py-2 px-2 border border-gray-300">
            <button className="bg-red-500 text-white px-2 py-1 rounded cursor-pointer" onClick={() => onDeleteItem(item.aggregatedId)}>
