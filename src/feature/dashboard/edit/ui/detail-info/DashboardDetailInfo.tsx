...
const DashboardDetailInfo: React.FC = () => {
  const { currentDashboard, groupItems, aggregatedItems, setGroupItems, setAggregatedItems, getDatabaseColumnList } = useDashboardStore();

  useEffect(() => {
    if (currentDashboard?.dashboardDetailInfo) {
      const { groupData, aggregatedData } = currentDashboard.dashboardDetailInfo;
  ...
  const handleAggregatedDataChange = (newAggregatedData: AggregatedData[]): void => {
  ...
  return (
    <>
      <div className="my-6">
        ...
        <DashboardGroupTable groupData={groupItems} onGroupDataChange={handleGroupDataChange} getDatabaseColumnList={getDatabaseColumnList} />
        <div className="mt-4 p-3 bg-gray-50 rounded text-sm text-gray-600">
          <p className="font-medium mb-2">현재 그룹 데이터:</p>
          <pre className="text-xs bg-white p-2 rounded border overflow-auto">{JSON.stringify(groupItems, null, 2)}</pre>
        </div>
        ...
        <DashboardAggregatedTable aggregatedData={aggregatedItems} onAggregatedDataChange={handleAggregatedDataChange} getDatabaseColumnList={getDatabaseColumnList} />
        <div className="mt-4 p-3 bg-green-50 rounded text-sm text-green-600">
          <p className="font-medium mb-2">현재 집계 데이터:</p>
          <pre className="text-xs bg-white p-2 rounded border overflow-auto">{JSON.stringify(aggregatedItems, null, 2)}</pre>
        </div>
        <div className="mt-4 p-3 bg-blue-50 rounded text-sm text-blue-600">
          <p className="font-medium mb-2">사용 방법:</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
